...
class StudentModel(nn.Module):
    ...
    def forward(self, texts, lengths, edu_nums, speakers, turns):
        ...
        sentences = torch.nn.functional.dropout(sentences, p=0.2, training=self.training)

        sentences = torch.cat((self.root.expand(batch_size, 1, sentences.size(-1)),
                               sentences.reshape(batch_size, edu_num, -1)), dim=1)

        dialogue, _ = self.gru(sentences)
        dialogue = self.dropout(dialogue)
        ...
        nodes = self.norm(sentences + dialogue)
...
class TeacherModel(nn.Module):
    ...
    def forward(self, texts, lengths, edu_nums, speakers, turns, graphs):
        ...
        edu_attn_mask = edu_attn_mask.unsqueeze(1).expand(batch_size, node_num, node_num).reshape(batch_size * node_num,
                                                                                                  node_num)
        ...
        const_path = self.path_emb(speakers, turns).unsqueeze(1).expand(batch_size, node_num, node_num, node_num,
                                                                        self.path_hidden_size).reshape(
            batch_size * node_num, node_num, node_num, self.path_hidden_size)
...
class PathDistiller(nn.Module):
    def __init__(self, params):
        super(PathDistiller, self).__init__()
        self.mapping = nn.Linear(params.path_hidden_size, params.hidden_size)

    def forward(self, teacher, student, mask):
        return torch.nn.functional.mse_loss(self.mapping(student[mask]), teacher[mask], reduction='sum') / torch.sum(
            mask)
