...
from model import StudentModel, PathClassifier, TeacherModel, Bridge
...
if __name__ == '__main__':
    ...
    parser.add_argument('--split_long_dialogue', action='store_true')
    parser.add_argument('--dialogue_max_length', type=int, default=80)
    parser.add_argument('--remake_dataset', action="store_true")
    parser.add_argument('--remake_tokenizer', action="store_true")
    parser.add_argument('--max_edu_dist', type=int, default=-1)
    # model
    ...
    parser.add_argument('--additional_loss', action='store_true')
    # save
    ...
    parser.add_argument('--classify_ratio', type=float, default=0.1)
    parser.add_argument('--distill_loss', action='store_true')
    parser.add_argument('--distill_ratio', type=float, default=1.)
    parser.add_argument('--early_stop',type=int, default=5)
    parser.add_argument('--task', type=str, default='student')
    ...
    if args.do_train:
        ...
        if args.classify_loss:
            path_classifier = PathClassifier(params=args)
            param_groups.append({'params': path_classifier.parameters(),'lr':args.learning_rate})
            path_classifier = path_classifier.to(args.device)
        ...
        for epoch in range(args.epoches):
            ...
            for batch in tqdm(train_dataloader):
                for mini_batch in batch:
                    ...
                    link_loss, label_loss, negative_loss = compute_loss(link_scores, label_scores, graphs, mask, negative=True)
                    ...
                    if args.classify_loss:
                        ...
                        cls_mask[:,:,0]=False
                        target=graphs
                        for layer_num, layer_path in enumerate(memory[:-1]):
                            classify_loss += path_classifier(layer_path, target, cls_mask)
                    ...
                    if args.task=='distill':
                        with torch.no_grad():
                            teacher_link_scores, teacher_label_scores, teacher_memory = t_model.forward(texts, lengths,
                                                                                                        edu_nums,
                                                                                                        speakers, turns,
                                                                                                        graphs)
                        padding_mask = mask
                ...
                if (total_step + 1) % args.report_step == 0:
                    print(
                        '\t{} step loss: {:.4f} {:.4f}, distill loss: {:.4f}, classify loss: {:.4f}'.format(
                            total_step + 1,
                    ...
                    accum_train_link_loss, accum_train_label_loss, accum_distill_loss, accum_classify_loss = 0, 0, 0, 0
                total_step += 1
            ...
            elif stop_sign>=args.early_stop:
                break
        ...
        f1_bi, f1_multi = tsinghua_F1(eval_matrix)
        print("\n---tsinghua eval result---"
              "\nlink micro-f1 : {}"
              "\nlabel micro-f1: {}\n".format(f1_bi, f1_multi))

        # p_data_file = os.path.join(args.sample_dir, 'test.pt')
        # torch.save(p_dict, p_data_file)
