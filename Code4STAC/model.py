...
class StudentModel(nn.Module):
    ...
    def forward(self, texts, lengths, edu_nums, speakers, turns):
        ...
        dialog_input = torch.cat(
            (self.root.expand(batch_size, 1, dialog_input.size(-1)), dialog_input.reshape(batch_size, edu_num, -1)),
            dim=1)
        ...
        # predicted_path = torch.cat((struct_path, const_path), -1)
...
class TeacherModel(nn.Module):
    def __init__(self, params, pretrained_embedding):
        ...
        self.link_classifier = Classifier(params.path_hidden_size+params.hidden_size * 4 , params.hidden_size, 1)
        self.label_classifier = Classifier(params.path_hidden_size+params.hidden_size * 4 , params.hidden_size,
                                           params.relation_type_num)
    ...
    def forward(self, texts, lengths, edu_nums, speakers, turns, graphs):
        ...
        dialog_input = torch.cat((self.root.expand(batch_size, 1, dialog_input.size(-1)),
                                  dialog_input.reshape(batch_size, edu_num, -1)),dim=1) # B N H
