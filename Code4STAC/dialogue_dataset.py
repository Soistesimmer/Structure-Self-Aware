...
class DialogueDataset(Dataset):
    def __init__(self, args, filename, mode, tokenizer):
        ...
        if mode == 'train' and args.split_long_dialogue:
            self.dialogues = self.split_too_long_dialogue(dialogues, args.dialogue_max_length)
    ...
    # too long dialogue impact the gpu memory used
    # split long dialogue to smaller ones
    def split_too_long_dialogue(self, dialogues, gate):
        num = len(dialogues)
        i = 0
        print("split long dialogue to shorter ones (training mode only)..")
        while i < num:
            edu_num = len(dialogues[i]['edus'])
            if edu_num > gate:
                dialogue = dialogues[i]
                dialogue_pieces = [{}, {}]
                weight = np.zeros(edu_num, dtype=np.long)
                for relation in dialogue['relations']:
                    x = relation['x']
                    y = relation['y']
                    weight[x:y] += 1
                a = 2 * edu_num // 5
                b = 3 * edu_num // 5
                p = a + np.argmin(weight[a:b])
                print(p, edu_num, weight[p])
                dialogue_pieces[0]['edus'] = dialogue['edus'][:p]
                dialogue_pieces[1]['edus'] = dialogue['edus'][p:]
                dialogue_pieces[0]['relations'] = []
                dialogue_pieces[1]['relations'] = []
                for relation in dialogue['relations']:
                    if relation['y'] < p:
                        if relation['x'] >= p:
                            continue
                        dialogue_pieces[0]['relations'].append(relation)
                    else:
                        relation['y'] -= p
                        if relation['x'] < p:
                            continue
                            ...
                            relation['x'] -= p
                        dialogue_pieces[1]['relations'].append(relation)
                dialogues[i] = dialogue_pieces[0]
                dialogues.append(dialogue_pieces[1])
            i += 1
        return dialogues
